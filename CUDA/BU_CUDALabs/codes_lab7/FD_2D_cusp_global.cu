...
__global__ void compute_RHS (float *RHS, float *u, int N, float h, float dt, float alpha, float T_bound)
{
	...
	if ( (G_J>N-3) || (G_I>N-3)){return;}	// N-3 porque G_J y G_I recorren el arreglo pequeño de 
						// de tamaño (N-2)x(N-2), que va de 0 a N-3 en cada 
						// dirección

	//bool bound_check = ((Index>N) && (Index< N*N-1-N) && (Index%N!=0) && (Index%N!=N-1)); 
	// No hay necesidad de usar el bound_check porque Index nunca toca los bordes inferiores pues esta definido
	// como i+1, j+1 ni los superiores debido al if de la linea 56

	RHS[Index2] = u[Index] + alpha*dt/(2*h*h) * (u[Index+1] + u[Index-1] + u[Index+N] + u[Index-N] - 4*u[Index]);
