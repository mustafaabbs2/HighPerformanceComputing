...
{
    int tid = (blockDim.x * blockDim.y * threadIdx.z) + (blockDim.x * threadIdx.y) + threadIdx.x; // Z | Y | X
 
    int nThreadsInBlock = blockDim.x * blockDim.y * blockDim.z;
    int block_offset_x = blockIdx.x * nThreadsInBlock; //how to advance in a row major direction - add number of threads in a block in the x direction 
    ...
    int nThreadsInRect = nThreadsInLine * gridDim.x; // number of threads in one whole rectangular frame 
